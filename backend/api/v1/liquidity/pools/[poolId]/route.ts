...
import { UpdatePoolInputSchema } from '@/schemas/liquidity/schema';
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  return executeRoute(request, { requiredPermission: 'liquidity.read' }, async (_req, context) => {
    ...
    const result = await services.liquidity.getPool(poolId);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  return executeRoute(request, { requiredPermission: 'liquidity.write' }, async (_req, context) => {
    const { poolId } = await params;
    const body = await parseJsonBody(request, UpdatePoolInputSchema);
    const services = buildServices(toServiceContext(context));
    const result = await services.liquidity.updatePool(poolId, body);
    return ok(parseResponse(result, PoolDetailSchema), context.requestId);
  });
}
